# Authors and Citation

## Authors

- **Andre Leite**. Author, maintainer.

- **Marcos Wasilew**. Author.

- **Hugo Vasconcelos**. Author.

- **Carlos Amorin**. Author.

- **Diogo Bezerra**. Author.

- **Júlia Nascimento Barreto**. Author.

## Citation

Source:
[`DESCRIPTION`](https://github.com/aurora-govpe/aurora-rpkg/blob/v0.1.9/DESCRIPTION)

Leite A, Wasilew M, Vasconcelos H, Amorin C, Bezerra D, Nascimento
Barreto J (2026). *aurora: Build Stateless Web Apps with 'plumber2'*. R
package version 0.1.9, <https://github.com/aurora-govpe/aurora-rpkg>.

    @Manual{,
      title = {aurora: Build Stateless Web Apps with 'plumber2'},
      author = {Andre Leite and Marcos Wasilew and Hugo Vasconcelos and Carlos Amorin and Diogo Bezerra and Júlia {Nascimento Barreto}},
      year = {2026},
      note = {R package version 0.1.9},
      url = {https://github.com/aurora-govpe/aurora-rpkg},
    }
