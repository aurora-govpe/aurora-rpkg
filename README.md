...
bslib/shiny at runtime. `aurora_build_image()` targets `linux/amd64` by
...
Apps that share static assets (logo, JS libraries, CSS) can serve them from one
server-side directory via `_aurora.yml`:
...
Mount the directory as a volume (`-v /srv/aurora-shared:/srv/aurora-shared:ro`)
and every app picks up changes from one place. See `vignette("deploy")`.
...
- Marcos Wasiliew · Hugo Vasconcelos · Carlos Amorim · Diogo Bezerra · Júlia Nascimento Barreto
