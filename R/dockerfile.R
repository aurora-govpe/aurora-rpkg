...
dockerfile_debian <- function(name, base, sysdeps, pkg_vec, port, aurora_source,
                              tz = NULL, locale = "C.UTF-8") {
  sys_lines <- paste0("    ", sysdeps, " \\", collapse = "\n")
  lang_env <- paste0("ENV LANG=", locale, " LC_ALL=", locale,
                     if (!is.null(tz) && nzchar(tz)) paste0(" TZ=", tz))
  # The rocker/Ubuntu base generates only en_US.UTF-8, so a specific locale must
  # be generated or R falls back to C. (C/C.UTF-8/POSIX are always available.)
  localegen <- if (!is.null(locale) && nzchar(locale) &&
                   !toupper(locale) %in% c("C", "C.UTF-8", "POSIX")) {
    paste0(
      "\n# Generate the requested locale (the base ships only en_US.UTF-8).\n",
...
dockerfile_alpine <- function(name, base, build_deps, runtime_deps, pkgs, port, aurora_source,
  ...
  # r-minimal strips R's bundled zoneinfo, so R needs TZDIR pointed at the tzdata
  # package for the timezone to resolve (otherwise it warns + falls back to UTC).
  lang_env <- paste0("ENV LANG=", locale, " LC_ALL=", locale,
                     if (!is.null(tz) && nzchar(tz)) paste0(" TZ=", tz, " TZDIR=/usr/share/zoneinfo"))
  glue::glue(
...
#'   Defaults to the pinned release `"aurora-govpe/aurora-rpkg@v0.1.10"` for
...
#'   `"America/Recife"`; pass `NULL` (or `""`) to omit the line. On `"alpine"`
#'   the image also gets `tzdata` and `TZDIR` so the zone actually resolves.
#' @param locale Locale baked as `ENV LANG=`/`LC_ALL=`. Defaults to `"pt_BR.UTF-8"`.
#'   `"C.UTF-8"` is UTF-8 and available everywhere (no generation needed). A specific locale like `"pt_BR.UTF-8"` needs
...
aurora_dockerfile <- function(dir = ".",
                              ...
                              sysdeps = "auto",
                              port = 8000L,
                              aurora_source = "aurora-govpe/aurora-rpkg@v0.1.10",
                              tz = "America/Recife",
                              locale = "pt_BR.UTF-8",
                              write = TRUE) {
  ...
  if (flavor == "debian") {
    ...
    df <- dockerfile_debian(cfg$name, base, sysdeps, pkg_vec, port, aurora_source, tz = tz, locale = locale)
