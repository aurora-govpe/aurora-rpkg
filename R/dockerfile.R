...
dockerfile_debian <- function(name, base, sysdeps, pkg_vec, port, aurora_source, tz = NULL) {
  sys_lines <- paste0("    ", sysdeps, " \\", collapse = "\n")
  lang_env <- paste0("ENV LANG=C.UTF-8 LC_ALL=C.UTF-8",
...
dockerfile_alpine <- function(name, base, build_deps, runtime_deps, pkgs, port, aurora_source, tz = NULL) {
...
#'   Defaults to the pinned release `"aurora-govpe/aurora-rpkg@v0.1.3"` for
...
aurora_dockerfile <- function(dir = ".",
  ...
  if (flavor == "debian") {
    ...
    df <- dockerfile_debian(cfg$name, base, sysdeps, pkg_vec, port, aurora_source, tz = tz)
